...
public class Nivel4 implements Pantalla {
	...
	private String rutaDisparo = "imagenes//disparo.png";
	...
	private Sonidos meeseek, laser;
	private boolean inmune, congelacion, metralleta;
	private String tipoPower;

	public Nivel4(PanelJuego panelJuego, JFrame ventana) {
	...
	public void inicializarPantalla() {
		...
		this.laser = new Sonidos("sonidos//laser.mp3");
		// Bola
		for (int i = 0; i < 2; i++) {
		...
		// Fondo
		try {
			this.fondo = ImageIO.read(new File("imagenes//nivel4.jpg"));
	...
	public void renderizarPantalla(Graphics g) {
		...
		pintarDisparo(g);
		if (bolas.size() <= 0) {
			panelJuego.setPantalla(new Victoria4(panelJuego, tiempoDeJuego, ventana, puntos));
		}
	}
		...
		if (!inmune) { // Power up -> Inmune
			for (int j = 0; j < bolas.size(); j++) {
				if (personaje.colisionan(bolas.get(j))) {
					panelJuego.setPantalla(new GameOver4(panelJuego, tiempoDeJuego, ventana, puntos));
				}
			}
		}
	}
