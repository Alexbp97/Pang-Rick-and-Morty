...
public class Nivel1 implements Pantalla {
	...
	public Nivel1(PanelJuego panelJuego, JFrame ventana) {
	...
	public void inicializarPantalla() {
		...
		// Fondo
		try {
			this.fondo = ImageIO.read(new File("imagenes//nivel1.png"));
	...
	public void renderizarPantalla(Graphics g) {
		...
		pintarDisparo(g);
		if (bolas.size() <= 0) {
			panelJuego.setPantalla(new Victoria1(panelJuego, tiempoDeJuego, ventana, puntos));
		}
	}
		...
		if (!inmune) { // Power up -> Inmune
			for (int j = 0; j < bolas.size(); j++) {
				if (personaje.colisionan(bolas.get(j))) {
					panelJuego.setPantalla(new GameOver1(panelJuego, tiempoDeJuego, ventana, puntos));
				}
			}
		}
	}
