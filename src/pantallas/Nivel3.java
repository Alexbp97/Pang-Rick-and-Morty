...
public class Nivel3 implements Pantalla {
	...
	private String rutaBola = "imagenes//bola3.png";
	private String personajeQuieto;
	private String rutaAndarDerecha;
	private String rutaAndarIzquierda;
	private String rutaDisparo = "imagenes//disparo1.png";
	...
	private Sonidos cromulon, lanzar;
	private boolean inmune, congelacion, metralleta;
	private String tipoPower;

	public Nivel3(PanelJuego panelJuego, JFrame ventana) {
	...
	public void inicializarPantalla() {
		...
		this.tipoPower = "";
		// Sonido
		this.cromulon = new Sonidos("sonidos//show_me.mp3");
		...
		bolas.add(new Sprite(ANCHO_BOLA, ALTO_BOLA, 10, 10, rd.nextInt(11) + 1, rd.nextInt(11) + 21, rutaBola, 3));
		// Fondo
		try {
			this.fondo = ImageIO.read(new File("imagenes//nivel3.png"));
	...
	public void renderizarPantalla(Graphics g) {
		...
		pintarDisparo(g);
		if (bolas.size() <= 0) {
			panelJuego.setPantalla(new Victoria3(panelJuego, tiempoDeJuego, ventana, puntos));
		}
	}
		...
		if (!inmune) { // Power up -> Inmune
			for (int j = 0; j < bolas.size(); j++) {
				if (personaje.colisionan(bolas.get(j))) {
					panelJuego.setPantalla(new GameOver3(panelJuego, tiempoDeJuego, ventana, puntos));
				}
			}
		}
	}
